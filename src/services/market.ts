    ...
    async function load() {
      try {
        ...
        const [mk, btc] = await Promise.all([
          withCache(`mk-${ids.length}`, () => geckoMarkets(ids), 60_000, 'cc.snapshots.daily').catch((e) => { throw e; }),
        ...
        const data: MarketData[] = CRYPTO_ASSETS.map((a) => {
          ...
          return {
            ...
            price, change1h: m?.price_change_percentage_1h_in_currency ?? c1,
        ...
        // fallback: tenta snapshot local
        try {
          const raw = localStorage.getItem('cc.snapshots.daily');
          if (raw) {
            const snap = JSON.parse(raw) as { data: unknown; ts: number };
            void snap;
          }
        } catch { /* sem snapshot */ }
